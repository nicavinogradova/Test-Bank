import random
import uuid
from typing import Any, get_type_hints, get_origin, Annotated, Literal, get_args
    ...
    def generate(cls: type, **overrides) -> Any:
        ...
        for field_name, annotated_type in type_hints.items():
            ...
            if get_origin(actual_type) is Annotated:
                actual_type, *annotations = get_args(actual_type)
                for ann in annotations:
                    if isinstance(ann, CreationRule):
                        rule = ann

            if get_origin(actual_type) is Literal:
                value = random.choice(get_args(actual_type))
